...
public class OPLTest {
    ...
    public void testCreateParties() {
        String[] electionData = {
                "OPL", "5", "Royce(D),Bob(R),Jerry(I),Lincoln(L),Timothy(D)", "6", "1", "1,2,,,"
        };
        OPL opl = new OPL(electionData);

        assertEquals(4, opl.parties.length);
        String[] expectedPartyNames = {"D", "R", "I", "L"};
        for (int i = 0; i < expectedPartyNames.length; i++) {
            assertEquals(expectedPartyNames[i], opl.parties[i].getName());
        }
    }
